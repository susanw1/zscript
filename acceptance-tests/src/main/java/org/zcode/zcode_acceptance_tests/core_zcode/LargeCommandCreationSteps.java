...
public class LargeCommandCreationSteps {
    ...
    public void the_target_is_sent_an_echo_command_with_as_much_data_as_possible() {
        ...
        int fieldl = ZcodeAcceptanceTestCapabilityResult.getMaxFieldNum() - 1;
        if (fieldl == -1) {
            fieldl = 255;
        }
    ...
    public void the_target_is_sent_a_capability_request_command() throws InterruptedException, ExecutionException, TimeoutException {
        assertThatCommand(ZcodeAcceptanceTestConnectionManager.getConnections().get(0), "R2\n").worked().hasField('M').getField('M', b -> {
        ...
        CoreZcodeSteps.testExpectedFields = a -> {
            for (int i = 0; i < ZcodeAcceptanceTestCapabilityResult.getMaxPageNum(); i++) {
                a = a.worked().hasBigField().getBigField(b -> {
                    ...
                    assertThat(ZcodeAcceptanceTestCapabilityResult.getGeneralCapabilities()).describedAs("Target must give consistant capabilities").isIn(-1,
                    ...
                    assertThat(ZcodeAcceptanceTestCapabilityResult.getMaxBigFieldLength()).describedAs("Target must give consistant capabilities").isIn(-1, size);
                    ...
                    assertThat(ZcodeAcceptanceTestCapabilityResult.getMaxFieldNum()).describedAs("Target must give consistant capabilities").isIn(-1, Byte.toUnsignedInt(n[0]));
                    ...
                    assertThat(ZcodeAcceptanceTestCapabilityResult.getMaxFieldSize()).describedAs("Target must give consistant capabilities").isIn(-1, Byte.toUnsignedInt(f[0]));
                    ...
                    assertThat(ZcodeAcceptanceTestCapabilityResult.getPersistantMemorySize()).describedAs("Target must give consistant capabilities").isIn((long) -1, size);
                    ZcodeAcceptanceTestCapabilityResult.setPersistantMemorySize(size);
                });
    ...
    public void the_target_is_sent_an_echo_command_with_too_much_field_data() {
        ...
        CoreZcodeSteps.testAssert = assertThatCommand(ZcodeAcceptanceTestConnectionManager.getConnections().get(0), "R1" + fieldStr + "\n");
    }
