...
public class RCodeAcceptanceTestResponse {
    private final RCodeAcceptanceTestFieldMap map = new RCodeAcceptanceTestFieldMap();
    private final RCodeAcceptanceTestBigField big = new RCodeAcceptanceTestBigField();
    private RCodeAcceptanceTestResponseStatus status = RCodeAcceptanceTestResponseStatus.OK;
    private char end;

    public RCodeAcceptanceTestFieldMap getFields() {
        return map;
    }

    public RCodeAcceptanceTestBigField getBigField() {
        return big;
    }

    public RCodeAcceptanceTestResponseStatus getStatus() {
        return status;
    }
    ...
    public String parseCommand(PushbackInputStream in, RCodeAcceptanceTestResponseSequence sequence, String current) {
        try {
            int c = in.read();
            while (c == ' ' || c == '\t' || c == '\r') {
                c = in.read();
            ...
            if (c == '#') {
                if (sequence.isEmpty()) {
                    status = RCodeAcceptanceTestResponseStatus.SKIP_COMMAND;
                    while (c != -1 && c != '\n') {
                        c = in.read();
                    }
                    end = '\n';
