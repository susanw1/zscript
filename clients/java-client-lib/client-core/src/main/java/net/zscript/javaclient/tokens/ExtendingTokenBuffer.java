...
public class ExtendingTokenBuffer extends AbstractArrayTokenBuffer {
    ...
    public static ExtendingTokenBuffer tokenize(ByteString sequence, boolean autoNewline) {
        ...
        for (int i = 0; i < sequence.size(); i++) {
            ...
            if (flags.getAndClearSeqMarkerWritten()) {
                byte key = buf.getTokenReader().tokenIterator().stream().filter(ReadToken::isSequenceEndMarker).findFirst().orElseThrow().getKey();
    ...
    public ExtendingTokenBuffer(ByteString tokenBytes) {
        super(validateTokens(tokenBytes).toByteArray(), tokenBytes.size());
    }

    /**
     * Validates a ByteString as containing a legitimate set of token bytes, as if generated by the tokenizer for some input. Note that the structure of the token sequence means
     * that we can only really check that the Marker/token lengths create a valid chain to the end of the bytes.
     *
