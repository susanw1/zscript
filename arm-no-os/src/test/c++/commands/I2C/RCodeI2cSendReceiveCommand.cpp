/*
 * RCodeI2cSendReceiveCommand.cpp
 *
 *  Created on: 12 Apr 2021
 *      Author: robert
 */

#include "RCodeI2cSendReceiveCommand.hpp"

void RCodeI2cSendReceiveCommand::setAsFinished(I2cTerminationStatus status, RCodeCommandSlot<RCodeParameters> *slot, uint8_t retries) {
...
void RCodeI2cSendReceiveCommand::moveAlong(RCodeCommandSlot<RCodeParameters> *slot) const {
    ...
    bus->asyncTransmitReceive(address, tenBit, slot->getBigField()->getData(), slot->getBigField()->getLength(), length, slot,
            &RCodeI2cSendReceiveCommand::setAsFinished,
            bus->getCallbackData() - 1);
}

void RCodeI2cSendReceiveCommand::finish(RCodeCommandSlot<RCodeParameters> *slot, RCodeOutStream<RCodeParameters> *out) const {
...
void RCodeI2cSendReceiveCommand::execute(RCodeCommandSlot<RCodeParameters> *slot, RCodeCommandSequence<RCodeParameters> *sequence,
    ...
    if (slot->getBigField()->getLength() == 0) {
        ...
        RCodeI2cSubsystem::getRCodeBus(bus)->asyncTransmitReceive(address, tenBit, NULL, 0, length, slot, &RCodeI2cSendReceiveCommand::setAsFinished,
                retries - 1);
    } else {
        RCodeI2cSubsystem::getRCodeBus(bus)->activateBus();
        RCodeI2cSubsystem::getRCodeBus(bus)->asyncTransmitReceive(address, tenBit, slot->getBigField()->getData(), slot->getBigField()->getLength(), length,
                slot, &RCodeI2cSendReceiveCommand::setAsFinished, retries - 1);
    }
}

void RCodeI2cSendReceiveCommand::setLocks(RCodeCommandSlot<RCodeParameters> *slot, RCodeLockSet<RCodeParameters> *locks) const {
    if (slot->getFields()->countFieldSections('B') <= 1) {
