    ...
    public void the_target_is_running_and_connected() {
        if (!hasStarted) {
            ...
            try {
                ...
                do {
                    Integer port = Integer.getInteger("rcode.network.acceptance-tests.port");

                    if (port == null) {
                        port = promptUserForPort();
                    }

                    String deviceIdent = System.getProperty("rcode.network.acceptance-tests.device-ident", null);
