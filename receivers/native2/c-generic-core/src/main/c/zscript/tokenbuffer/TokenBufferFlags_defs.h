...
/**
 * @brief Creates a freshly initialized ZStok_BufferFlags *by value*.
 * @return a freshly initialized ZStok_BufferFlags
 */
static ZStok_BufferFlags zstok_createBufferFlags(void) {
    return (ZStok_BufferFlags) {
            .markerWritten = false,
            .seqMarkerWritten= false,
            .readerBlocked= true,
            .fatalBufferError= false
    };
}

/**
 * @brief Signals that some Marker (possibly a sequence marker) has just been written to the TokenBuffer.
 *
...
/**
 * @brief Indicates if any Marker (sequence or otherwise) has been written (flag is auto-cleared).
 *
...
/**
 * @brief Signals that some sequence marker has just been written to the TokenBuffer.
 *
...
/**
 * @brief Indicates whether a *sequence* Marker has been written (flag is auto-cleared).
 *
...
/**
 * @brief Indicates whether the Reader is blocked (ie waiting for a Marker to say it can proceed with processing).
 *
...
/**
 * @brief Used by a Reader to signal that it is now blocked - it has processed everything it can and is awaiting more tokens.
 *
...
/**
 * @brief Used by a Reader to signal that it is not (or no longer) blocked, and can proceed with processing tokens if given the chance.
 *
 * This is effectively an acknowledgement that a Marker has been identified.
 *
...
/**
 * @brief Indicates whether the Buffer has detected a usage error or been put in an unstable state.
 *
 * This should be checked and raised as an error condition.
 *
...
/**
 * @brief Signals that the buffer is in an error state. Check \zs_errno for specifics.
 */
