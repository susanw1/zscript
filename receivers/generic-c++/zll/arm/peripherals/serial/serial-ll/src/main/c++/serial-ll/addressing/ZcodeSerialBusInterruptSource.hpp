...
class ZcodeSerialBusInterruptSource: public ZcodeBusInterruptSource<ZP> {
    ...
    static void callback(SerialIdentifier id) {
        if (!SerialManager<LL>::isMasked(id)) {
...
public:
    ...
    ZcodeSerialBusInterruptSource() {
        for (uint16_t i = 0; i < SerialManager<LL>::serialCount; ++i) {
            SerialManager<LL>::getSerialById(i)->setTargetValue(&ZcodeSerialBusInterruptSource::callback, EOL_SYMBOL);
        }
    }

    ZcodeNotificationInfo takeUncheckedNotification() {
        if (serialIntReceivedBits != 0) {
            for (uint8_t i = 0; i < SerialManager<LL>::serialCount; ++i) {
