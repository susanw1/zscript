...
#ifndef PERIPHERALS_SERIAL_SRC_MAIN_C___SERIAL_ZCODESERIALCHANNEL_HPP_
#define PERIPHERALS_SERIAL_SRC_MAIN_C___SERIAL_ZCODESERIALCHANNEL_HPP_
...
public:
    SerialChannel(SerialIdentifier id) :
            ZcodeCommandChannel<ZP>(&seqin, &out, false), seqin(this, SerialManager < LL > ::getSerialById(id)), out(SerialManager < LL > ::getSerialById(id)) {
        SerialManager < LL > ::maskSerial(id);
        seqin.getSerial()->init(NULL, 9600, false);
    }

    SerialChannel(SerialIdentifier id, uint32_t baud) :
            ZcodeCommandChannel<ZP>(&seqin, &out, false), seqin(this, SerialManager < LL > ::getSerialById(id)), out(SerialManager < LL > ::getSerialById(id)) {
        SerialManager < LL > ::maskSerial(id);
    ...
    SerialChannel(Serial *serial) :
        ...
        SerialManager < LL > ::maskSerial(serial->getId());
        seqin.getSerial()->init(NULL, 9600, false);
    }
...
#endif /* PERIPHERALS_SERIAL_SRC_MAIN_C___SERIAL_ZCODESERIALCHANNEL_HPP_ */
