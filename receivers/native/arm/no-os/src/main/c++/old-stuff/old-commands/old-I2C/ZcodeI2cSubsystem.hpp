...
class ZcodeI2cSubsystem {
    private:
        ...
        static ZcodeI2cBus buses[4 * ZcodePeripheralParameters::i2cBusesPerPhyBus];
    ...
    public:
        static void init() {
            ...
            for (int i = 0; i < ZcodePeripheralParameters::i2cBusesPerPhyBus; ++i) {
                buses[i * 4].setup(&phyBus0, i * 4, ZcodeParameters::i2cPhyBus0Lock);
                buses[i * 4 + 1].setup(&phyBus1, i * 4 + 1, ZcodeParameters::i2cPhyBus1Lock);
                buses[i * 4 + 2].setup(&phyBus2, i * 4 + 2, ZcodeParameters::i2cPhyBus2Lock);
                buses[i * 4 + 3].setup(&phyBus3, i * 4 + 3, ZcodeParameters::i2cPhyBus3Lock);
            }
        }
