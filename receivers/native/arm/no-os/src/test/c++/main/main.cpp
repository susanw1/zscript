...
int main(void) {
    ...
    Ucpd<LowLevelConfiguration>::init(&notSoPermanentStore, 150, 5 * 20, 5 * 20);
    ...
    AtoDManager<LowLevelConfiguration>::init();
    SystemMilliClock<LowLevelConfiguration>::blockDelayMillis(1000);
    Usb<LowLevelConfiguration>::usb.init(NULL, 0, false);

    EthernetUdpChannel<ZcodeParameters> channel(4889);
    SerialChannel<ZcodeParameters> serial(&Usb<LowLevelConfiguration>::usb);
    ...
    I2c<LowLevelConfiguration> *i2c1 = I2cManager<LowLevelConfiguration>::getI2cById(0);
    if (i2c1->lock()) {
