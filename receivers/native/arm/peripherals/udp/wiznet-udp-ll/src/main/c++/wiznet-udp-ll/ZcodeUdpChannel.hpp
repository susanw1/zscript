...
#include <wiznet-udp-ll/WiznetUdpLLInterfaceInclude.hpp>
...
template<class ZP>
class ZcodeUdpChannelInStream: public ZcodeChannelInStream<ZP> {
private:
...
public:
    ZcodeUdpChannelInStream(ZcodeCommandChannel<ZP> *channel, EthernetUDP<LL> *udp) :
...
template<class ZP>
class ZcodeUdpChannel;

template<class ZP>
class ZcodeUdpOutStream: public ZcodeOutStream<ZP> {
private:
    typedef typename ZP::LL LL;
    struct UdpOutStatus {
        bool openB :1;
    ...
    ZcodeUdpChannel<ZP> *channel;
    uint8_t readBuffer[ZP::ethernetUdpChannelReadBufferSize];
    UdpOutStatus status;

public:
    ZcodeUdpOutStream(ZcodeUdpChannel<ZP> *channel) :
...
template<class ZP>
class ZcodeUdpChannel: public ZcodeCommandChannel<ZP> {
    typedef typename ZP::LL LL;
    EthernetUDP<LL> udp;
    ZcodeUdpChannelInStream<ZP> seqin;
    ZcodeUdpOutStream<ZP> out;
...
template<class ZP>
void ZcodeUdpOutStream<ZP>::actualOpen() {
    status.openFrozen = false;
    ZcodeUdpChannelInStream<ZP> *in = (ZcodeUdpChannelInStream<ZP>*) channel->in;
