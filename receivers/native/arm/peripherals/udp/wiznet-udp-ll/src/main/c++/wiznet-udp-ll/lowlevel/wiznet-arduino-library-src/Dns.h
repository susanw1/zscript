...
template<class LL>
void DNSClient<LL>::begin(const IPAddress &aDNSServer)
        {
...
template<class LL>
int DNSClient<LL>::inet_aton(const char *address, IPAddress &result)
        {
...
template<class LL>
int DNSClient<LL>::getHostByName(const char *aHostname, IPAddress &aResult, uint16_t timeout)
        {
    int ret = 0;
...
template<class LL>
uint16_t DNSClient<LL>::BuildRequest(const char *aName)
        {
    // Build header
    //                                    1  1  1  1  1  1
