...
public:
    ScriptSpace(uint8_t *scriptSpace, uint16_t spaceLength, uint8_t *outBuffer, uint16_t outBufferSize) :
            ZscriptChannel<ZP>(buffer.getReadBuffer(), &out), buffer(scriptSpace, spaceLength), out(&this->parser, outBuffer, outBufferSize), canBeWrittenToB(true) {
        stop();
    }
    ScriptSpace(uint8_t *scriptSpace, uint16_t spaceLength, uint16_t initialDataLength, bool canWrite, uint8_t *outBuffer, uint16_t outBufferSize) :
            ZscriptChannel<ZP>(buffer.getReadBuffer(), &out), buffer(scriptSpace, spaceLength, initialDataLength, canWrite), out(&this->parser, outBuffer, outBufferSize), canBeWrittenToB(
                    canWrite) {
    }
